				...
				<table class="tbl-ex">
					...
					<tr>
						...
						<c:if test="${vo.depth > 0}">
							<td style="padding-left:${40 * vo.depth}px; text-align:left;">
								<img src='${pageContext.servletContext.contextPath }/assets/images/reply.png'/>
								<a href="${pageContext.servletContext.contextPath}/board?a=viewform&no=${vo.no}&username=${vo.userName}">${vo.title}</a>
							</td>
						</c:if>
						<c:if test="${vo.depth == 0}">
							<td>
						...
						<c:choose>
							...
							<c:when test="${kwd == null && pageInfo.totalRows != 0}">
								<c:choose>
									...
									<c:otherwise>
										<li><a href="${pageContext.servletContext.contextPath}/board?a=list&page=${(pageInfo.currentBlock-1)*pageInfo.pagesPerBlock}">◀</a></li>
									</c:otherwise>
								</c:choose>
										...
										<c:forEach begin="1" end="${pageInfo.pagesPerBlock}" varStatus="num">
											<c:choose>
												<c:when test="${num.count == pageInfo.currentPage}">
													<li class="selected"><a href="${pageContext.servletContext.contextPath}/board?a=list&page=${(pageInfo.currentBlock - 1) * pageInfo.pagesPerBlock + num.count}">${(pageInfo.currentBlock- 1) * pageInfo.pagesPerBlock + num.count}</a></li>
												</c:when>
												<c:otherwise>
													<li><a href="${pageContext.servletContext.contextPath}/board?a=list&page=${(pageInfo.currentBlock - 1) * pageInfo.pagesPerBlock + num.count}">${(pageInfo.currentBlock- 1) * pageInfo.pagesPerBlock + num.count}</a></li>
												</c:otherwise>
											</c:choose>
	                       				</c:forEach>
									</c:when>
								...
								<c:choose>
									...
									<c:otherwise>
										<li><a href="${pageContext.servletContext.contextPath}/board?a=list&page=${pageInfo.currentBlock * pageInfo.pagesPerBlock + 1 }">▶</a></li>
									</c:otherwise>
								</c:choose>
							</c:when>
